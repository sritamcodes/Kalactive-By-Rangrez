...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $items) {
    ...
    if ($name === '') {
        ...
        try {
            ...
            $stmt = $conn->prepare("INSERT INTO orders (user_id, customer_name, customer_email, customer_address, total_amount, payment_method, status) VALUES (?, ?, ?, ?, ?, 'cod', 'pending')");
            $stmt->execute([$user['id'] ?? null, $name, $email, $address, $freshCart['total']]);
